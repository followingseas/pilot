...
| `pilot release template <name>` | dry-run 렌더 — 파일을 쓰지 않고 산출물 출력 |
| `pilot release upgrade <name> [--approve-locked-field-change]` | revision 증가와 함께 갱신 — locked field 변경은 명시 승인 없이는 실패 |
...
| `adapters.claude/codex/copilot` | X | `{ enabled, output }` — 렌더 대상 파일 (기본: `CLAUDE.md` · `AGENTS.md` · `.github/copilot-instructions.md`, copilot은 기본 비활성) |

### PolicySet (canonical policy IR)

`policies/*.yaml`은 `kind: PolicySet`으로 기계 검증 가능한 규칙을 선언한다. rule의 최소 필드는 `id` · `level`(error/warn/info) · `statement`이고, `rationale` · `examples` · `checks` · `remediation`을 함께 둘 수 있다. rule은 adapter 산출물(`## 핵심 규칙`)과 MCP `pilot_get_policy` 응답에 그대로 반영된다.
...
| `.pilot/release.yaml` | 설치 상태 — release 이름, revision, 산출물 checksum, 이전 revision. 커밋 대상 |
